...
docker compose down -v  # также удалить тома с данными
```
...
Это распространяется и на одновременные запросы. При повторном использовании ключа возвращается исходный платёж, даже если тело запроса изменилось. Длина ключа — от 1 до 255 символов.

Сумма должна быть положительной, содержать не более двух знаков после запятой и помещаться в `NUMERIC(18, 2)`. Некорректные данные возвращают `422`, отсутствующий или неверный API-ключ — `401`, неизвестный платёж — `404`.
...
Три попытки включают первоначальную; задержки перед повторами — 1 и 2 секунды. Успешная доставка webhook требует ответа `2xx`. HTTP-ошибки и тайм-ауты приводят к повтору. Статус `failed` означает результат эмуляции платежа; ошибка доставки webhook не изменяет уже сохранённый результат платежа.

Доставка допускает дубликаты: при сбое между публикацией и фиксацией outbox событие может быть отправлено повторно. Обработчик блокирует строку платежа при обработке и сохраняет первоначальный результат. Получателю webhook нужно учитывать возможные повторные уведомления по `payment_id`.
